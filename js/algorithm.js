...
function calculateF() {
  for (p in readyQueue) {
    readyQueue[p].f =
      (1.0 * (3 * readyQueue[p].priority + readyQueue[p].burstTimePriority)) /
      4;
  }
}
...
async function FCFS(flag) {
  ...
  outer: while (readyQueue.length != 0) {
    ...
    if (flag) {
      ...
      $("#vis_cpu")
        .empty()
        .html(`<span class='fitem'>P${processQueue[p].id}</span>`);
      $("#vis_time").empty().append(time);
      $(".btn").attr("disabled", true);
      await new Promise((r) => setTimeout(r, 2000));
      if (stop_flag) break outer;
  ...
  outer: while (processQueue.length != 0) {
    ...
    if (paused) {
      await new Promise((resolve) => {
        continueExecution = resolve; // Store the resolve function
      });
    }
    if (
    ...
    if (flag1) {
      ...
      $("#vis_cpu")
        .empty()
        .html(`<span class='fitem'>P${currentProcess.id}</span>`);
      $("#vis_time").empty().append(time);
      $(".btn").attr("disabled", true);
      await new Promise((r) => setTimeout(r, 2000));
      if (stop_flag) break outer;
...
// Function to toggle between pausing and resuming the scheduling process
function toggleScheduling() {
  paused = !paused; // Toggle the paused state
  const resumeBtn = document.getElementById("vis_resume");
  if (paused) {
    resumeBtn.textContent = "Resume"; // Change button text to "Resume"
  } else {
    resumeBtn.textContent = "Pause"; // Change button text to "Pause"
    if (continueExecution) {
      continueExecution(); // Resolve the promise to continue execution
    }
  }
}
var resultTable;
...
function displayGanttChart() {
  for (i in ganttFCFS) {
    ...
    if (ganttFCFS[i].processId != null)
      ...
      $("#gantt_FCFS").append(
        `<div class="gantt-box" style="width: ${diff}px"><span class="gantt-box-left">${ganttFCFS[i].startTime}</span><span class="gantt-box-right">${ganttFCFS[i].endTime}<span></div>`
      );
  }
  $("#gantt_FCFS").append("<br><br>"); // Add space after FCFS Gantt chart
  ...
  for (i in ganttSJFNonPre) {
    ...
    if (ganttSJFNonPre[i].processId != null)
      ...
      $("#gantt_SJFNonPre").append(
        `<div class="gantt-box" style="width: ${diff}px"><span class="gantt-box-left">${ganttSJFNonPre[i].startTime}</span><span class="gantt-box-right">${ganttSJFNonPre[i].endTime}<span></div>`
      );
  }
  $("#gantt_SJFNonPre").append("<br><br>"); // Add space after SJF Non-Preemptive Gantt chart
  ...
  for (i in ganttSJFPre) {
    ...
    if (ganttSJFPre[i].processId != null)
      ...
      $("#gantt_SJFPre").append(
        `<div class="gantt-box" style=" width: ${diff}px"><span class="gantt-box-left">${ganttSJFPre[i].startTime}</span><span class="gantt-box-right">${ganttSJFPre[i].endTime}<span></div>`
      );
  }
  $("#gantt_SJFPre").append("<br><br>"); // Add space after SJF Preemptive Gantt chart
  ...
  for (i in ganttPriorityNonPre) {
    ...
    if (ganttPriorityNonPre[i].processId != null)
      ...
      $("#gantt_PriorityNonPre").append(
        `<div class="gantt-box" style="width: ${diff}px"><span class="gantt-box-left">${ganttPriorityNonPre[i].startTime}</span><span class="gantt-box-right">${ganttPriorityNonPre[i].endTime}<span></div>`
      );
  }
  $("#gantt_PriorityNonPre").append("<br><br>"); // Add space after Priority Non-Preemptive Gantt chart
  ...
  for (i in ganttPriorityPre) {
    ...
    if (ganttPriorityPre[i].processId != null)
      ...
      $("#gantt_PriorityPre").append(
        `<div class="gantt-box" style="width: ${diff}px"><span class="gantt-box-left">${ganttPriorityPre[i].startTime}</span><span class="gantt-box-right">${ganttPriorityPre[i].endTime}<span></div>`
      );
  }
  $("#gantt_PriorityPre").append("<br><br>"); // Add space after Priority Preemptive Gantt chart
  ...
  for (i in ganttRoundRobin) {
    ...
    if (ganttRoundRobin[i].processId != null)
      ...
      $("#gantt_RoundRobin").append(
        `<div class="gantt-box" style="width: ${diff}px"><span class="gantt-box-left">${ganttRoundRobin[i].startTime}</span><span class="gantt-box-right">${ganttRoundRobin[i].endTime}<span></div>`
      );
  }
  $("#gantt_RoundRobin").append("<br><br>"); // Add space after Round Robin Gantt chart

  for (i in ganttProposed) {
    let diff = ganttProposed[i].endTime - ganttProposed[i].startTime + 80;
    if (ganttProposed[i].processId != null)
      $("#gantt_Proposed").append(
        `<div class="gantt-box" style="width:${diff}px"><span class="gantt-box-left">${ganttProposed[
          i
        ].startTime.toFixed(1)}</span>P${
          ganttProposed[i].processId
        }<span class="gantt-box-right">${ganttProposed[i].endTime.toFixed(
          1
        )}<span></div>`
      );
  }
  $("#gantt_Proposed").append("<br><br>"); // Add space after Proposed Gantt chart
}
  ...
  for (a in algorithms) {
    if (checked[a] && rank[a] === minRank) {
      bestAlgo.push({
        ...
        throughput:
          (throughput[a] * 100).toFixed(2) + " % (Process per unit time)",
        tat: tat[a].toFixed(2),
        wt: wt[a].toFixed(2),
        rt: rt[a].toFixed(2),
      });
